...
def extractToolNodes(tool_nodes_path):
    tool_nodes_dic = {}
    toolNodesScarler_file = os.path.dirname(tool_nodes_path) + '/tool_nodes_attr.csv'
    ...
    df = tool_nodes_df.drop(['id', 'tool'], axis=1)
    ...
    df_score.to_csv(toolNodesScarler_file, index=False)


def extractSampleNodeAttr(df_data, sample_nodes_attr_path):
    df_node_attr = df_data[['purity', 'shortCNV', 'middleCNV', 'largeCNV', 'deletion', 'readLen', 'readDepth', 'sample']]
...
def extractNodes(df_data,workdir):
    sample_nodes_path = workdir + '/data/sample_nodes.csv'  #'F:/CNVrecommendation/newCalling/sample_nodes.csv'
    tool_nodes_path = workdir + '/data/tool_nodes.csv' #'F:/CNVrecommendation/newCalling/tool_nodes.csv'
    sample_nodes_attr_path = workdir + '/data/sample_nodes_attr.csv' #'F:/CNVrecommendation/newCalling/sample_nodes_attr.csv'
...
def dfStandardScaler(df_data,ExtractingMetaTargetScarler):
    # zscore = preprocessing.MinMaxScaler()
    # # df = df_data.drop(['sample', 'Stype', 'Ftype', 'Ptype'], 1)

    # df = df_data.drop('sample', axis=1)
    ...
    # df_score.to_csv(ExtractingMetaTargetScarler, index=False)
    df_data.to_csv(ExtractingMetaTargetScarler, index=False)


def mian(ExtractingMetaTarget_df, workdir):

    ExtractingMetaTargetScarler = workdir + '/data/ExtractingMetaTargetScarler.csv'
    if not os.path.exists(ExtractingMetaTargetScarler):
        dfStandardScaler(ExtractingMetaTarget_df, ExtractingMetaTargetScarler)
    # else:
    #     print(ExtractingMetaTargetScarler)
    #     exit()
    ExtractingMetaTargetScarler_df = pd.read_csv(ExtractingMetaTargetScarler, encoding='gbk', low_memory=False)
    extractNodes(ExtractingMetaTargetScarler_df, workdir)


if __name__ == '__main__':
    ExtractingMetaTarget = '../data/ExtractingMetaTarget.tsv'
    ExtractingMetaTargetScarler = '../data/ExtractingMetaTargetScarler.csv'
