...
from CNVrecom.CNVrecomSST.metaPathDGLT import extractGraphData

__all__ = ['FtypeDataDglDatasetT']
...
class FtypeDataDglDatasetT(DGLDataset):
    def __init__(self, seed=0,save_graph=True):

        self.seed = seed
        self.save_graph = save_graph
        self.save_mydata = '/Volumes/MyBook/2023work/CNVrecommendation/exomeData/dglDataset'
        super(FtypeDataDglDatasetT, self).__init__(name="FtypeDataDglDatasetT")
    ...
    def has_cache(self):
        ...
        graph_path = os.path.join(self.save_mydata, 'SST_dgl_graph_{}{}.bin'.format(self.hash, 'FtypeData'))
    ...
    def load(self):
        graphs, label_dict = load_graphs(os.path.join(self.save_mydata, 'SST_dgl_graph_{}{}.bin'.format(self.hash, 'FtypeData')))
        self.graphs = graphs
