...
from metaPathDGL import extractGraphData

__all__ = ['StypeDataDglDataset']
...
class StypeDataDglDataset(DGLDataset):
    def __init__(self, seed=0,save_graph=True):

        self.seed = seed
        self.save_graph = save_graph
        self.save_mydata = 'J:/DATA/CNVrecommendData/'
        super(StypeDataDglDataset, self).__init__(name="StypeDataDglDataset")

    def process(self):

        b = extractGraphData()
        tag = 'S'
        graphs_list, label_list,  = b.getDataList(tag)
    ...
    def has_cache(self):
        ...
        graph_path = os.path.join(self.save_mydata, 'STT_dgl_graph_{}{}.bin'.format(self.hash, 'StypeData'))
    ...
    def load(self):
        graphs, label_dict = load_graphs(os.path.join(self.save_mydata, 'STT_dgl_graph_{}{}.bin'.format(self.hash, 'StypeData')))
        self.graphs = graphs
